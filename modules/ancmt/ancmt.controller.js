"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.AnnouncementsController = void 0;
const common_1 = require("@nestjs/common");
const swagger_1 = require("@nestjs/swagger");
const ancmt_service_1 = require("./ancmt.service");
const create_announcement_1 = require("./dto/create-announcement");
let AnnouncementsController = class AnnouncementsController {
    constructor(ancmtService) {
        this.ancmtService = ancmtService;
    }
    create(botId, chatId, body) {
        return this.ancmtService.createAnnouncement(botId, chatId, body.text);
    }
};
__decorate([
    (0, swagger_1.ApiQuery)({
        name: 'botId',
        required: true,
    }),
    (0, swagger_1.ApiQuery)({
        name: 'chatId',
        required: true,
    }),
    (0, common_1.Post)(),
    __param(0, (0, common_1.Query)('botId')),
    __param(1, (0, common_1.Query)('chatId')),
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [String, String, create_announcement_1.CreateAnnouncementDTO]),
    __metadata("design:returntype", void 0)
], AnnouncementsController.prototype, "create", null);
AnnouncementsController = __decorate([
    (0, swagger_1.ApiTags)('announcements'),
    (0, common_1.Controller)('announcements'),
    __metadata("design:paramtypes", [ancmt_service_1.AnnouncementsService])
], AnnouncementsController);
exports.AnnouncementsController = AnnouncementsController;
//# sourceMappingURL=ancmt.controller.js.map